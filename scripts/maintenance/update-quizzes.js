...
for (let i = 0; i < sections.length; i += 2) {
    ...
    if (match) {
        ...
        try {
            ...
            const indentedJson = rawJson.split('\\n').map((line, idx) => {
                if (idx === 0) return line;
                return '    ' + line;
            }).join('\\n');

            // Fixed Regex Construction
            const regex = new RegExp('("id":\\s*"' + storyId + '"[^]*?"quizzes":\\s*)(\\[[^]*?\\])', 'g');

            // Perform replacement
            if (regex.test(fileContent)) {
                fileContent = fileContent.replace(regex, '$1' + indentedJson);
                console.log(`Updated ${storyId}`);
            } else {
                console.log(`Could not find block for ${storyId}`);
            }

        } catch (e) {
            console.error(`Failed to parse JSON for ${storyId}: ${e.message}`);
        }
    }
}
