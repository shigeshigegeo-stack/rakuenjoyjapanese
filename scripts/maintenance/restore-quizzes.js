...
// Parse inputData to get a map of ID -> Quizzes JSON string
const storyQuizzes = {};
    ...
    if (match) {
        ...
        try {
            ...
            const indentedLines = lines.map((line, idx) => {
                if (idx === 0) return line; // nested indentation
                return '    ' + line;
            });
            storyQuizzes[storyId] = indentedLines.join('\\n');
        } catch (e) { }
    }
}

// Now replace in fileContent
// Pattern: "quizzes": [ ...  },
// We match: ("quizzes":\s*\[)([^]*?)(\n  \},)
// Warning: Previous script truncated the file, so [^]*? might match until next story start if we are not careful.
// But we know 'next story start' or 'current story end' is signified by `\n  },`.
// Since we have `  },` at the end of the truncated block (from previous script's block logic).
// We can assume `\n  },` is the safe delimiter.

let newFileContent = fileContent;

for (const storyId of Object.keys(storyQuizzes)) {
    // Regex: Match id...quizzes...tail
    // Tail ends with \n  },

    const regex = new RegExp('("id":\\s*"' + storyId + '"[^]*?"quizzes":\\s*)([^]*?)(\\n  \\},)');

    if (regex.test(newFileContent)) {
        const correctQuizzes = storyQuizzes[storyId];
        // Replacer: Group 1 (Head) + Correct Quizzes + \n  } (Closing)
        // Group 3 matches `\n  },` (with comma).
        // Since we are rebuilding, we want to ensure the story object ends cleanly.
        // We will output `\n  }` without comma, and let the loop or join handle commas?
        // Wait, the file structure relies on commas between objects.
        // But our `clean_stories_v2` joined blocks with `,\n`.
        // So the `\n  },` in the current file HAS a comma.
        // We will preserve `$3` which is the closing token.

        newFileContent = newFileContent.replace(regex, '$1' + correctQuizzes + '$3');
